...
Aus diesem Grund wird getestet, ob ein Modell mit `sprgeb_red_x` sich signifikant von einem ohne diesem Prädiktor unterscheidet, falls alle anderen Prädiktoren im Modell sind (Tabelle \ref{tab:lrt.negbin.spr}). Der Test zeigt, dass sich die beiden Modelle nicht signifikant voneinander unterscheiden. Als Folge wird auf den Prädiktor `sprgeb_red_x` verzeichtet.
...
In Tabelle \ref{tab:lrt.negbin.age} ist ersichtlich, dass der quadratische Term für den Prädiktor Alter das Modell verbessert.

```{r lrt.negbin.age, echo=FALSE}
glm.negbin.age <- glm.nb(formula = kons_n ~ valter_n, data = ads.1.mod)
glm.negbin.age2 <- update(glm.negbin.all, . ~ . + I(valter_n^2))
...
glm.negbin.pcg2 <- update(glm.negbin.all, . ~ . + I(pcg_n^2))
...
Als potentieller Effektmodifikator wird die Farnchisestufe verwendet. Es soll untersucht werden, ob die Restriktionen zu einem Managed-Care Modell sowohl bei tiefen, als auch bei hohen Franchisen einen Effekt zur Folge haben.
