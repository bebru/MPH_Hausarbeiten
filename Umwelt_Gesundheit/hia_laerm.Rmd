...
die Lärmemissionen durch den Eisenbahnverkehr reduziert. Dies ist die Intevention, 
...
Mortalität angeschaut. Gemssen werden diese mit Hilfe der Anzahl Spitaltage (Morbidität), 
...
L~den~, und können deshalb ohne weitere Annahmen oder Umformngen miteinander in Beziehung 
gesetzt werden.  
...
Die Referenzwerte für die Anzahl Spitaltage resp. Todesfälle aufgrund von Herzifarkten stammen aus 
...
(Spitaltage), resp. ischämischen Herzkrankheiten (Todesfälle) zu finden. Bei den Todesfällen wird hier 
angenommen, dass Herzinfarkte den grössten Teil dieser rapporitierten Fälle ausmachen, 
und deshalb diese Angaben so verwendet werden können. Bei den Spitaltagen wird davon ausgegangen, dass 
Herzinfarkte für die Hälfte dieser Spitaltage verantwortlich sind. Zudem wird angenommen, dass sich 
die Spitaltage auf eine Schweizer Bevölkerung von 8.3 Mio. Personen beziehen.  
In Tabelle \ref{tbl:tbl.dis} sind die Angaben zu der Anzahl Spitaltage/Todesfällen, bezogen auf 
jeweils 100'000 Personen, aufgeführt.  
...
in der betrachteten Population der Anzeil berechnet werden, welche aufgrund der 
...
mit einem L~den~ von 50-60 dB(A) belastet sind 26 Spitaltage pro Jahr eingespart werden. 
In der Population mit L~den~ > 60 dB(A) können 11 Spitaltage pro Jahr eingespart werden. 
Insgesamt spart die Intervention (pro Jahr) somit 37 Spitaltage aufgrund von Herzinfarkten 
ein.
...
tbl.death <- "
  Kennzahl   | Population 50-60 dB | Population >60 dB | Bemerkung
  N          | 165'000             | 35'000            | aus Ingold und Köpfli 2009
  d          | 93.8                | 93.8              | pro 100'000 Personen, aus Todesfallstatistik
  HR         | 1.020               | 1.020             | pro 10 dB Anstieg, aus Héritier et al. (2017)
  Delta~int~ | -1                  | -2                | Annahme Auswirkung der Intervention
  RR         | 0.998               | 0.996             | 
  p~ex~      | 1                   | 1                 | ganze Population
...
Durch die Intervention können somit in der Population welche vor der Intervention 
mit einem L~den~ von 50-60 dB(A) belastet sind 0.31 Todesfälle eingespart werden. 
In der Population mit L~den~ > 60 dB(A) können 0.13 Todesfälle eingespart werden. 
Insgesamt spart die Intervention (pro Jahr) somit 0.44 Todesfälle aufgrund von Herzinfarkten 
ein.


## Sensitivitätsanalyse

Sämtliche in dieser HIA verwendeten Parameter unterliegen einer Unsicherheit. Um 
den Einfluss dieser Unsicherheit auf die Resultate zu untersuchen ist es sinnvoll, 
eine Sensitivitätsanalyse durchzuführen. Dabei wird üblicherweise für jeweils einen 
der Parameter andere Werte verwendet, und die so entstanden Resultate miteinander 
verglichen:
...
Streuung aufweisen, und das untere Ende des entsprechenden Konfidenzinervalls 
sehr nahe bei 1, also keiner Wirkung, ist (HR = 1.020 (1.007–1.033)).
...
sicht dieser Einfluss nicht entscheidend auf das Resultat auswirkt.
* Die Messung der Anzahl Spitaltage und Todesfälle dürfte von den verwendeten Parametern 
am genauesten sein. Allerdings beziehen sich diese nicht direkt auf das gewünschte 
Outcome Herzinfarkt, sondern allgemeiner auf ischämische Herzkrankheiten. Somit könnte 
bei diesen beiden Parametern auch mit leicht tieferen Werten gerechnet werden.
...
um 1 resp. 2 dB). Somit ist es plausibel, dass die geschätzte Wirkung der Intervention 
auf die Anzahl Spitaltage ebenso wie auf die Anzahl Todesfälle (0.44 pro Jahr) sehr tief ist.
